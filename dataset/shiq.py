...
class SHIQ_Dataset(LightningDataModule):
    def __init__(
        self,
        data_dir: str,
        batch_size: int,
        ...
        self.t_highlight = transforms.Compose(
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.train_data = ImageFolders('train', self.train_root, self.scale_size, self.crop_size,
                                           ...
                                           ['highlight', 'mask', 'free'], 
                                           [self.t_highlight, self.t_mask, self.t_mask], 
                                           self.return_name)
...
if __name__ == '__main__':
    ...
    dst = SHIQ_Dataset(r'../../SHIQ_Dataset', 1, 0, True, (448, 448), (400, 400))
    dst.setup()
    ds = dst.val_dataloader()
    for i, d in enumerate(ds):
        print(d['highlight'].shape, d['mask'].shape, d['free'].shape, d['name'])
        '''
        plt.subplot(131)
        plt.imshow(d['highlight'].squeeze().permute(1,2,0).numpy())
        plt.subplot(132)
